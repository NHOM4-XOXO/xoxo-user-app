...
const MainPost = ({ data, reactionStats, currentUserId }) => {
    ...
    const handleMenuClick = async (key) => {
        if (key === "edit") {
            setEditingPost(data); // truyền đúng post
            setIsModalOpen(true);
        }
        if (key === "delete") {
            try {
                ...
                toast.error("Xóa bài viết thất bại" + error?.data?.message ? `: ${error.data.message}` : ".");
            }
        }
    };
    ...
    return (
        <>
            ...
            {/* Reactions */}
            <PostFooter post={{ ...post, commentCount: data?.post.commentCount }} reactionStats={reactionStats} />
