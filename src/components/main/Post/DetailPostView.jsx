...
import {
    X, ThumbsUp, MessageCircle, Share2, Globe, UsersRound,
    Lock, MoreHorizontal, ChevronLeft, ChevronRight
} from "lucide-react";
...
export default function DetailedVideoView({ postId }) {
    ...
    const getPrivacyIcon = (privacy) => {
        switch (privacy) {
            case "public": return <Globe className="w-3 h-3 text-gray-600 dark:text-gray-400" />;
            case "friends": return <UsersRound className="w-3 h-3 text-gray-600 dark:text-gray-400" />;
            case "onlyMe": return <Lock className="w-3 h-3 text-gray-600 dark:text-gray-400" />;
            default: return <Globe className="w-3 h-3 text-gray-600 dark:text-gray-400" />;
        }
    };
    ...
    return (
        <div className={`flex flex-col lg:flex-row max-h-[100vh-${HEADER_HEIGHT}px] h-full bg-fb-light-secondary dark:bg-fb-dark-primary text-black dark:text-white`}>
                ...
                <div className="max-w-[90vw] max-h-[90vh] flex items-center justify-center">
                    {currentMedia?.type === "video" ? (
                        <video
                            ...
                            poster={currentMedia.thumbnail || "/placeholder.svg?height=720&width=1280"}
                            controls
                            autoPlay
                            playsInline
                ...
                <div className="p-4">
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 mb-3">
                        <div className="flex items-center space-x-2">
                            <span className="hover:underline cursor-pointer">{likesCount} lượt thích</span>
                            <span>·</span>
                            <span className="hover:underline cursor-pointer">{post.commentsCount} bình luận</span>
                            <span>·</span>
                            <span className="hover:underline cursor-pointer">{post.views} lượt xem</span>
                        </div>
                    </div>
