...
export default function WatchSidebar({ itemSelected, setItemSelected }) {
  ...
  const handleHistoryItemClick = (item) => {
    setSearchTerm(item);
    setShowHistory(false);
    // TODO: có thể trigger search ngay ở đây
  };
  ...
  return (
    <SidebarShell
      ...
      contentClassName="gap-4 relative"
    >
      {/* Header */}
      <div className="px-4 py-2 flex items-center justify-between">
        <h2 className="text-2xl font-bold text-black dark:text-white">Video</h2>
      ...
      {/* Search history dropdown (absolute trong shell) */}
      {showHistory && (
        <div
          ref={historyDropdownRef}
          className="absolute left-4 right-4 top-28 md:w-[calc(100%-2rem)] bg-fb-light-primary dark:bg-fb-dark-secondary rounded-lg shadow-lg z-10"
        >
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.name} className="mt-1">
                {/* Dùng SidebarNavItem để đồng bộ hover/rounded/padding */}
                <SidebarNavItem
                  icon={
                    <div
                      className={[
